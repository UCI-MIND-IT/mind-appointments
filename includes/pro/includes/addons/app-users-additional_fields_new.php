...
?></span></label>
                    ...
                    <?php if ( 'select' === $field['type'] && ! empty( $field['options'] ) ) :
                        ...
                        foreach ( explode( ',', $field['options'] ) as $opt ) {
                            ...
                            if ( count( $parts ) === 2 ) {
                                $options_array[ trim( $parts[0] ) ] = trim( $parts[1] );
                            }
                        }
                    ?>
                        <select
                        ...
                        >
                            <?php foreach ( $options_array as $opt_value => $opt_label ) : ?>
...
<?php
	...
	public function show_settings( $style ) {
		...
		$_types = array(
            ...
            'select'   => __( 'Dropdown', 'appointments' ),
		);
		$options = appointments_get_options();
		$fields = $options['additional_fields'];
?>
                ...
                <tr valign="top" class="" <?php echo $style?>>
                    ...
                    <td colspan="2">
                        ...
                        <div id="app-new_additional_field">
                            ...
                            <label for="app-new_additional_field-options" id="app-new_additional_field-options-wrap" style="display:none;">
                                <?php _e( 'Options (format: value:Label, value2:Label 2):', 'appointments' ); ?>
                                <input type="text" value="" id="app-new_additional_field-options" placeholder="e.g. virtual:Virtual (Zoom), in_person:In Person" style="width:400px;" />
                            </label>
    ...
    $(function () {
        ...
        $(document).on("change", "#app-new_additional_field-type", function () {
            if ( $(this).val() === 'select' ) {
                $("#app-new_additional_field-options-wrap").show();
            } else {
                $("#app-new_additional_field-options-wrap").hide();
            }
        });
    });
